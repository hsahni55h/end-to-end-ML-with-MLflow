from dataclasses import (  # Import the dataclass decorator for creating data classes
    dataclass,
)
...
@dataclass(
    frozen=True
)  # Define the class as a dataclass and make it immutable with frozen=True
...
class DataValidationConfig:
    ...
    unzip_data_dir: (
        Path  # unzip_data_dir (Path): The directory where the unzipped data is stored.
    )
...
class ModelTrainerConfig:
    ...
    target_column: (
        str  # target_column (str): The name of the target column in the dataset.
    )
...
class ModelEvaluationConfig:
    ...
    test_data_path: Path  # test_data_path (Path): The path to the testing data.
    model_path: (
        Path  # model_path (Path): The path to the saved model that will be evaluated.
    )
